...
const ROOT = path.join(__dirname, isDev ? '../..' : '../../..')
...
// ------------ Game: engine process + WebSocket ------------

ipcMain.handle('create-room', async (_, opts: { mode: string; deckCount: number; playerCount: number }) => {
  if (engineProcess) {
  ...
  if (!wss) {
    ...
    wss.on('connection', (ws) => {
      ...
      ws.on('message', (data) => {
        ...
        if (engineProcess?.stdin && str.trim()) engineProcess.stdin.write(str.trim() + '\n')
      })
...
ipcMain.handle('join-room', async (_, hostIP: string) => {
  return new Promise((resolve) => {
    ...
    clientWs.on('close', () => { clientWs = null })
  })
})

ipcMain.handle('start-game', async (_, opts: { mode: string; deckCount: number; playerCount: number; players: { human: boolean }[]; seed?: number }) => {
  ...
  const players = opts.players ?? Array.from({ length: playerCount }, (_, i) => ({ human: i < humanCount }))
...
function sendToEngine(obj: object) {
...
ipcMain.handle('game-play', async (_, cards: string[], requestId?: string) => {
  const payload: Record<string, unknown> = { cmd: 'play', cards }
  if (requestId) payload.requestId = requestId
  return { ok: sendToEngine(payload) }
})

ipcMain.handle('game-pass', async (_, requestId?: string) => {
  const payload: Record<string, unknown> = { cmd: 'pass' }
  if (requestId) payload.requestId = requestId
  return { ok: sendToEngine(payload) }
})

ipcMain.handle('game-call', async (_, choice: string) => {
  return { ok: sendToEngine({ cmd: 'call', choice }) }
})

ipcMain.handle('game-discard', async (_, card: string) => {
  if (card && card.trim())
    return { ok: sendToEngine({ cmd: 'discard', card: card.trim() }) }
  return { ok: sendToEngine({ cmd: 'discard' }) }
})

async function getLocalAddresses(): Promise<string[]> {

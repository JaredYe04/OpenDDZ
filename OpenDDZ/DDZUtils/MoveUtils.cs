    ...
    public static class MoveUtils
    {
        ...
        private static Card ParseCard(string str)
        {
            ...
            if (string.IsNullOrEmpty(str))
                throw new ArgumentException("空牌面");

            if (str == "小王" || str.Equals("X", StringComparison.OrdinalIgnoreCase))
                return new Card(Rank.JokerSmall, Suit.Joker);
            if (str == "大王" || str.Equals("Y", StringComparison.OrdinalIgnoreCase))
                return new Card(Rank.JokerBig, Suit.Joker);

            Suit? suit = null;
            string rankStr = str;

            if (str.StartsWith("红桃")) { suit = Suit.Heart; rankStr = str.Substring(2); }
            else if (str.StartsWith("黑桃")) { suit = Suit.Spade; rankStr = str.Substring(2); }
            else if (str.StartsWith("方片")) { suit = Suit.Diamond; rankStr = str.Substring(2); }
            else if (str.StartsWith("梅花")) { suit = Suit.Club; rankStr = str.Substring(2); }
